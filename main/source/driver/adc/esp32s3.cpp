...
namespace
{
    ...
    */
    ...
    bool Esp32s3::init() noexcept 
    {
        ...
        if(!isAdcPin(myPin)) { return false; }

        // Create a reference to the ESP32-S3 pin manager.       
        auto &pinManager = system::pin_manager::Esp32s3::instance();

        // Try to book a GPIO pin via the pin manager, return false on failure.
        if (!pinManager.reservePin(myPin)) { return false; }
    ...
    float Esp32s3::readVoltage() const noexcept
    {
        ...
        const std::uint16_t rawValue = readRaw();

        // Convert raw ADC value to volts.
        return (static_cast<float>(rawValue) / 4095.0F) * 3.3F;
    }
