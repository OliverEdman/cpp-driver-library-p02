...
// -----------------------------------------------------------------------------
bool Adaptive::train(const std::size_t epochCount, double learningRate,
                  const double precisionThreshold) noexcept
{
    constexpr std::size_t evaluationInterval{10U};
    constexpr double maxLr{0.25};
    constexpr double minLr{1e-3};
    ...
    for (std::size_t epoch{}; epoch < epochCount; ++epoch)
    {
        ...
        if (evaluate)
        {
            ...
            // Calculate difference between precision and 
            const auto delta = precision - prevPrecision;
