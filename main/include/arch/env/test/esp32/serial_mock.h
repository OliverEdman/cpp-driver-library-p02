...
static inline esp_err_t uart_enable_pattern_det_baud_intr(uart_port_t uart_num, char pattern_chr,
...
static inline esp_err_t usb_serial_jtag_driver_install(const usb_serial_jtag_driver_config_t* config)
{
    (void)config;
    return ESP_OK;
}

static inline esp_err_t usb_serial_jtag_driver_uninstall(void)
{
    return ESP_OK;
}

static inline int usb_serial_jtag_write_bytes(const void* src, size_t size, uint32_t ticks_to_wait)
{
    (void)src;
    (void)ticks_to_wait;
    return static_cast<int>(size);
