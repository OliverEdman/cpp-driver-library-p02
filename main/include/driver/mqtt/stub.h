...
public:
     ...
     *
     *        This minimal stub currently has no queued messages, so this always returns false.
     */
    bool readMessage(char* topic, std::uint16_t topicMaxLen,
                     char* payload, std::uint16_t payloadMaxLen) noexcept override
    {
        (void)topic;
        (void)topicMaxLen;
        (void)payload;
