...
private:
    /** Maximum simulated buffer size. */
    static constexpr std::uint8_t BufSize{100U};

    /** Buffer used to store simulated incoming data. */
    std::uint8_t myBuf[BufSize];

    /** Current number of valid bytes stored in the buffer. */
    std::uint8_t myBufLen;

    /** Current read position index inside the buffer*/
    std::uint8_t myBufIndex;

    /** Flag tracking if data is ready to be read. */
    bool myDataAvailable;

    /** Flag tracking if the simulated device is connected*/
    bool myConnected;
};
}// namespace driver:: serial
